defmodule Aoc.Intcode do
    ...
    @doc """
    Await IO from an Intcode program. State is carried, and eventually returned.
    
    ## Options
    
     - `output_count` - Number of outputs to coalesce before sending to output_function
    ...
    def await_io(state, opts \\ []) do
        ...
        output_function = opts |> Keyword.get(:output_function, &default_output/2)
        input_function = opts |> Keyword.get(:input_function, &default_input/1)
        
        await_io_handle([], state, [output_count: output_count, output_function: output_function, input_function: input_function])
    end
    
    def await_io_handle(output_list, state, opts) do
        ...
        receive do
            ...
            # program is done - return
            :halt -> 
                if length(output_list) != 0 do
                    IO.puts("Intcode VM warning: output buffer still has data: #{output_list}")
                end
                state
            ...
            v -> 
                ol = output_list ++ [v]
                if length(ol) == output_count do
                    case output_function.(state, ol) do
                        {:continue, new_state} -> await_io_handle([], new_state, opts)
                        {:halt, new_state} -> new_state
                    end
                else
                    await_io_handle(ol, state, opts)
                end
        end
    end
    ...
    """
    def memory_write(program, address, value) when is_list(program) and is_integer(address) and is_integer(value) do
        program |> List.replace_at(address, value)
    end
    ...
    ## Example
        ...
        iex> run([109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99])
    ...
    def run(program, opts \\ []) do
        ...
        # initialize
        %{program: initialized_program, state: state} = initialize(program, memory_size: prog_size)
    ...
    def initialize(program, opts \\ []) do
        ...
        program_size = opts |> Keyword.get(:memory_size, 2000)
    ...
    def default_output(_state, v) do
        default_output(v)
    end
